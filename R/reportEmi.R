...
reportEmi <- function(gdx, output = NULL, regionSubsetList = NULL,
  ...
  # carbon stored in biochar (already negative in EmiPe2Se) FIND A BETTER PLACE
  ...
  if (any(vm_emiMacSector[,,"co2steel"] > 0)) {
    ...
    out <- mbind(out,
                 ...
                 # negative emissions from (non-ES) CDR (DACCS, EW, OAE)
  ...
  # and add non-geologic storage cdr
  ...
  out <- mbind(out,
               ...
               setNames(out[, , "Emi|CO2|Energy|Supply|+|Biochar w/ couple prod (Mt CO2/yr)"]
                        ...
                        "Emi|CO2|Gross|Energy|Supply|+|Biochar (Mt CO2/yr)"), # gross emissions of biochar are zero. Check whether important for summations
  ...
  if (!is.null(v37_plasticsCarbon)) {
    ...
    out <- mbind(out,
                 ...
                 setNames(out[, , "Emi|CO2|Energy and Industrial Processes (Mt CO2/yr)"]
                          - out[, , "Emi|CO2|CDR|Industry CCS|Synthetic Fuels (Mt CO2/yr)"]
                          - out[, , "Emi|CO2|CDR|BECCS (Mt CO2/yr)"]
                          - out[, , "Emi|CO2|CDR|Biochar (Mt CO2/yr)"], ## TO BE CHECKED IF CORRECT
                          "Emi|CO2|Gross|Energy and Industrial Processes (Mt CO2/yr)"),
  ...
  # non-Energy System CDR from CDR module
  out <-  mbind(out,
  ...
  if (!is.null(v37_plasticsCarbon)) {
    ...
    out <- mbind(out,
               ...
               setNames(
                 ...
                 # CDR energy-related emissions
                 (dimSums(mselect(EmiFeCarrier[, , "ETS"], emi_sectors = "CDR"), dim = 3)
                  # Captured CO2 by non-ES CDR technologies
                  + (vm_emiCdrTeDetail[, , "weathering"] 
                      + vm_emiCdrTeDetail[, , "dac"] * p_share_CCS
                      + vm_emiCdrTeDetail[, , "oae"] * (1 - s33_OAE_chem_decomposition * (1 - p_share_CCS))
                      )) * GtC_2_MtCO2,
