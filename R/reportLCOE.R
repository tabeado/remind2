...
#gdx <- "/p/tmp/tabeado/sebiochar_AprRelease/remind/output/PB650-all-CFmed2_2024-05-12_15.46.46/fulldata.gdx"
#output.type = "both"
...
#### A) Calculation of average (standing system) LCOE ----
 ...
 EW_name   <- "weathering" # necessary for backward compatibility. For versions prior to CDR module refactoring, it's overwritten below
 ...
 vm_emiCdrTeDetail <- readGDX(gdx, c("vm_emiCdrTeDetail","v33_emi"), field = "l", restore_zeros = F, react = "silent")[,ttot_from2005,teCDR]
  if (is.null(vm_emiCdrTeDetail)) { # compatibility with the CDR module before the refactoring
    # captured CO2 by DAC
